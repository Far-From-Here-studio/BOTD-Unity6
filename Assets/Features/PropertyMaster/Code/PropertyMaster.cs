...
public class PropertyMaster : MonoBehaviour, IExposedPropertyTable, ISerializationCallbackReceiver {
    ...
    protected void OnEnable() {
		RenderPipelineManager.beginCameraRendering += OnBeforeCameraCull;
    }
    ...
    void OnBeforeCameraCull(ScriptableRenderContext context, Camera camera) {
        if (updateMode == UpdateMode.Automatic && (camera.cameraType == CameraType.SceneView || camera.cameraType == CameraType.Game))
            UpdateProperties();
    }
