...
public class DepthOfFieldAutoFocus : MonoBehaviour//, IDepthOfFieldAutoFocus //UPGRADE-TODO
{
    static public DepthOfFieldAutoFocus Instance { get; private set; }
    ...
    struct AutoFocusOutput // Same As DepthOfFieldAutoFocus.compute
    {
        public float focusDistance;
        public float lensCoeff;
    };
    AutoFocusOutput[] m_AutoFocusOutput = new AutoFocusOutput[1];
    ...
    Camera m_Camera;
    [SerializeField]
    HDAdditionalCameraData m_HDCamera;
    ...
    void Reset()
    {
        ...
        m_HDCamera = GetComponent<HDAdditionalCameraData>();
    }
    ...
    private void BeginCameraRendering(ScriptableRenderContext context, Camera camera)
    {
        ...
        {
            m_AutoFocusOutputCB.GetData(m_AutoFocusOutput);
            GetDoFParams(out _, out var aperture);

            var physicalParameters = m_HDCamera.physicalParameters;
            physicalParameters.focusDistance = m_AutoFocusOutput[0].focusDistance;
            physicalParameters.aperture = aperture;
            m_HDCamera.physicalParameters = physicalParameters;

            //Debug.Log($"Focus Distance: {m_AutoFocusOutput[0].focusDistance}\n Lens Coeff: {m_AutoFocusOutput[0].lensCoeff}");
        }

        var cmd = new CommandBuffer();
        SetUpAutoFocusParams(cmd, camera.focalLength, camera.sensorSize.y / 1000f, camera, false);
