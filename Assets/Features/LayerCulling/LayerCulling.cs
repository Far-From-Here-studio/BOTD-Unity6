...
public class LayerCulling : MonoBehaviour {
...
#endif
		RenderPipelineManager.beginCameraRendering += UpdateLayerCulling;

	}

	static void UpdateLayerCulling(ScriptableRenderContext context, Camera camera) {
		...
		if(!ForceDisable && Instance && Instance.isActiveAndEnabled && (!isSceneView || ms_SceneViewCulling)) {
			AssignCulling(camera, Instance);
		} else {
			ClearLayerCulling(camera, Instance ? Instance.shadowCullTarget : null);
		}
	...
	static void AssignCulling(Camera cameraCullTarget, LayerCulling culling) {
		...
		/*if(cameraCullTarget)
			cameraCullTarget.layerCullSpherical = false;
		*/
		if(cameraCullTarget) {
